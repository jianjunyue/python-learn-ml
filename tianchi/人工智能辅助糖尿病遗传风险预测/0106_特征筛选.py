...
base_score=np.mean(test_score)

column_weigth={}
for column in tqdm(quantity):
    temp_X = X.drop([column], axis=1)
    clf = XGBRegressor()
    kfold = KFold(n_splits=5, random_state=7)
    test_score = np.sqrt(-cross_val_score(clf, temp_X, Y, cv=kfold, scoring='neg_mean_squared_error'))
    column_weigth[column]=np.mean(test_score)-base_score

# column_weigth= sorted(column_weigth.values(), key=lambda d:d[1], reverse = True)
print(column_weigth)


plt.bar(left= range(len(column_weigth)), height=column_weigth.values(), width=0.35, align="center", yerr=0.0001)

plt.xticks(range(len(column_weigth)),column_weigth.values())
